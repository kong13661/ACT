...
class TrainingStep(pl.LightningModule):
    def __init__(self, config):
        ...
        self.fid = FrechetInceptionDistance(normalize=True, reset_real_features=False, fid_path=config['fid_path'])
        if config['compile']:
    ...
    def r1_panelty(self, real_logits, real, t):
        ...
        grad_penalty = (torch.sum(torch.square(grad_real[0]), dim=[1, 2, 3]) * self.disc_weight(t)).mean()
    ...
    def disc_weight(self, t):
        ...
        weight = weight ** math.log(self.config['mid_weight'] / self.config['gan_ratio'], 0.5)
        return weight
    ...
    def discriminator_fake_loss(self, x):
        ...
        fake_image = self.augment(fake_image, self.ada_aug_p)[0].detach()
        fake_output_fx = self.model.discriminator(fake_image, t)
        fake_loss_fx = self.loss_func(fake_output_fx, torch.ones_like(fake_output_fx), t)
        return fake_loss_fx
    ...
    def discriminator_real_loss(self, x):
        ...
        x = self.augment(x, self.ada_aug_p)[0]
        real_output_fx = self.model.discriminator(x, t)

        real_loss_fx = self.loss_func(real_output_fx, - torch.ones_like(real_output_fx), t)
        r1_loss_fx = self.r1_panelty(real_output_fx, x, t) if self.config['r1_gamma'] > 0 else 0
    ...
    def generator_training_step(self, x):
        ...
        _, consistency_loss = self.consistency_loss(sample_x, sample_xt, next_t)
        _, g_loss = self.generator_loss(sample_x, next_t)
    ...
    def generator_loss(self, fake_image, t):
        ...
        fake_image = self.augment(fake_image, self.ada_aug_p)[0]
        fake_output_fx = self.model.discriminator(fake_image, t)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            ...
            xts = xts.reshape(-1, 5, *xts.shape[1:])
            ...
            xts_sample = xts_sample.reshape(-1, 5, *xts_sample.shape[1:])
...
def main(
    ...
    sigma_max                               = 80,
    rho                         : float     = 7,
    max_sigma_scale             : float     = 1.02,
    curve_rho                   : float     = 1,
    max_steps                   : int       = 800000,

    dataset_path                            = "/hpc2hdd/home/hcheng046/kongfei/dataset",
    ckpt_path                               = "/hpc2hdd/home/hcheng046/kongfei/models_state_dict",
    fid_path                                = "/hpc2hdd/home/hcheng046/kongfei/consistency_gan_new/VIRTUAL_lsun_cat256.npz",
