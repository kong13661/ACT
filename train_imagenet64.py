...
from models.discriminator_imagenet import UNet2DModelDiscriminator
...
from torchmetrics.image.fid import FrechetInceptionDistance as _FrechetInceptionDistance, _compute_fid
...
class FrechetInceptionDistance(_FrechetInceptionDistance):
    def __init__(self, feature=2048, reset_real_features=True, normalize=False, fid_path=None, **kwargs: Any):
        super().__init__(feature, reset_real_features, normalize, **kwargs)
        fid_cache = np.load(fid_path)
        self.register_buffer('real_mu', torch.tensor(fid_cache['mu']))
        self.register_buffer('real_cov', torch.tensor(fid_cache['sigma']))

    def compute(self):
        """Calculate FID score based on accumulated extracted features from the two distributions."""
        if self.fake_features_num_samples < 2:
            raise RuntimeError(
        ...
        return _compute_fid(self.real_mu.squeeze(0), self.real_cov, mean_fake.squeeze(0), cov_fake).to(self.orig_dtype)
...
class TrainingStep(pl.LightningModule):
    def __init__(self, config):
        ...
        self.distance_loss = get_distance_loss(
            config['consistency_distance'], config['compile'])

        self._r1_ema = config['r1_threshold']
        self.config = config

        self.consistency_loss = self._interval_wrapper(
            self.consistency_loss, 1, ('consistency_loss', 'none'))
        ...
        self.generator_loss = self._interval_wrapper(
            self.generator_loss, 1, ('gloss_fx', 'none'))
        ...
        self.fid = FrechetInceptionDistance(
            normalize=True, reset_real_features=False, fid_path=config['fid_path'])
        if config['compile']:
            self.fid = torch.compile(self.fid)

    def r1_panelty(self, real_logits, real, t):
        grad_real = torch.autograd.grad(
            outputs=real_logits.sum(), inputs=real, create_graph=True
        )
        grad_penalty = (torch.sum(torch.square(grad_real[0]), dim=[
                        1, 2, 3]) * self.disc_weight(t)).mean()
        r1_loss = grad_penalty * (self.config['r1_gamma'] * 0.5)
        self.update_r1_ema_and_aug_p(r1_loss.detach())
        return r1_loss

    def loss_func(self, x, y, t):
        return (torch.nn.functional.softplus(x * y) * self.disc_weight(t)).mean()

    def loss_func_g(self, x, y, t):
        return torch.nn.functional.softplus(x * y)

    def disc_weight(self, t):
        steps = self.model.time_to_timesteps(t, self.bins)
        weight = steps / (self.bins - 1)
        weight = weight ** math.log(self.config['mid_weight'] /
                                    self.config['gan_ratio'], 0.5)
        return weight
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_cal_gloss = y[:y.shape[0] // 2]
        x_cal_dloss = x[x.shape[0] // 2:]
        y_cal_dloss = y[y.shape[0] // 2:]
        ...
        for i in range(self.config['accumu']):
            ...
            loss_g = self.generator_training_step(
                x_cal_gloss[idx: idx_r], y_cal_gloss[idx: idx_r])
        ...
        for i in range(self.config['accumu']):
            ...
            *_, loss_d = self.discriminator_training_step(
                x_cal_dloss[idx: idx_r], y_cal_dloss[idx: idx_r])
    ...
    def discriminator_training_step(self, x, y):
        ...
        fake_loss_total = self.discriminator_fake_loss(x1, y1)
        real_loss_total, r1_loss = self.discriminator_real_loss(x2, y2)
        dloss_total = (fake_loss_total + real_loss_total)

        return dloss_total, (dloss_total + r1_loss) * self.config['gan_ratio']

    def discriminator_fake_loss(self, x, y):
        t = self.model.train_time_sampler(self.bins, x.shape[0])
        noise = torch.randn_like(x)
        xt = self.model.diffusion(x, t, noise)
        with torch.no_grad():
            fake_image = self.model(xt, t, y)

        fake_image = augment(fake_image, self.ada_aug_p)[0].detach()
        fake_output_fx = self.model.discriminator(fake_image, t, y)
        fake_loss_fx = self.loss_func(
            fake_output_fx, torch.ones_like(fake_output_fx), t)
        return fake_loss_fx

    def discriminator_real_loss(self, x, y):
        t = self.model.train_time_sampler(self.bins, x.shape[0])
        x = x.clone()
        x.requires_grad = True
        x = augment(x, self.ada_aug_p)[0]
        real_output_fx = self.model.discriminator(x, t, y)

        real_loss_fx = self.loss_func(
            real_output_fx, - torch.ones_like(real_output_fx), t)
        r1_loss_fx = self.r1_panelty(
            real_output_fx, x, t) if self.config['r1_gamma'] > 0 else 0
    ...
    def generator_training_step(self, x, y):
        ...
        with torch.no_grad():
            sample_xt = self.model_ema(xt, sample_t, y)
        sample_mixture = xt_next_noise

        sample_x = self.model(sample_mixture, next_t, y)

        _, consistency_loss = self.consistency_loss(
            sample_x, sample_xt, next_t)
        _, g_loss = self.generator_loss(sample_x, next_t, y)
        ...
        return torch.lerp(consistency_loss, g_loss, self.config['gan_ratio'] * self.disc_weight(next_t)).mean()
        # return g_loss

    def generator_loss(self, fake_image, t, y):
        if self.config['gan_ratio'] == 0:
            return torch.tensor(0., device=self.device)
        fake_image = augment(fake_image, self.ada_aug_p)[0]
        fake_output_fx = self.model.discriminator(fake_image, t, y)
        fake_loss_fx = self.loss_func_g(
            fake_output_fx, - torch.ones_like(fake_output_fx), t)
        weight = self.disc_weight(t)
        return (fake_loss_fx * weight).mean().detach(), fake_loss_fx

    def consistency_loss(self, target, pred, t):
        if self.config['consistency_loss_weight'] == 'none':
            loss = self.distance_loss(target, pred, 1)
        elif self.config['consistency_loss_weight'] == 'edm':
            weight = self.model.get_scalings_for_boundary_condition(t)[-1]
            loss = self.distance_loss(target, pred, weight)

        return loss.mean().detach(), loss
    ...
    def configure_optimizers(self):
        ...
        if self.optim == 'adam':
            optimizer_g = torch.optim.Adam(
                self.model.unet.parameters(), betas=(0, 0.99), lr=self.lr)
            optimizer_d = torch.optim.Adam(
                self.model._discriminator.parameters(), betas=(0, 0.99), lr=self.config['d_lr'])

        if self.optim == 'radam':
            optimizer_g = torch.optim.RAdam(
                self.model.unet.parameters(), lr=self.lr, betas=(0, 0.99))
            optimizer_d = torch.optim.RAdam(self.model._discriminator.parameters(
            ), lr=self.config['d_lr'], betas=(0, 0.99 * r))

        if self.optim == 'RMSprop':
            optimizer_g = torch.optim.RMSprop(
                self.model.unet.parameters(), lr=self.lr)
            optimizer_d = torch.optim.RMSprop(
                self.model._discriminator.parameters(), lr=self.config['d_lr'])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y = torch.randint(0, 1000, (x.shape[0],), device=x.device).long()
        x_sample = self.model_ema(noise, x.new_ones(
            [x.shape[0], ], dtype=torch.float32) * self.config['sigma_max'], y)
        ...
        if batch_idx == 0:
            ...
            label_y = label_y[:self.sample_size]
            x_grid = make_grid((x + 1) / 2)

            noise = torch.randn_like(x) * self.config['sigma_max']
            x_sample = self.model_ema(noise, x.new_ones(
                [x.shape[0], ], dtype=torch.float32) * self.config['sigma_max'], label_y)
            x_sample = x_sample.clip(-1, 1)
            x_sample_grid = make_grid((x_sample + 1) / 2)

            self.trainer.loggers[0].log_image(
                'dataset', [x_grid], self.trainer.global_step)
            self.trainer.loggers[0].log_image(
                'sample', [x_sample_grid], self.trainer.global_step)
            ...
            ts = []
            label_ys = []
            times = [(self.config['bins_max'] - i - 1) for i in range(0,
                                                                      self.config['bins_max'], self.config['bins_max'] // 10)]
            times.sort()
            noise = torch.randn_like(x)
            for t in times:
                xts.append((x + noise * self.model.timesteps_to_time(x.new_ones(
                    [x.shape[0], 1, 1, 1], dtype=torch.float32) * t, self.config['bins_max'])))
                label_ys.append(label_y[:6])
                ts.append(self.model.timesteps_to_time(x.new_ones(
                    [x.shape[0], ], dtype=torch.float32) * t, self.config['bins_max']))
            xts = torch.cat(xts, dim=0)
            ts = torch.cat(ts, dim=0)
            label_ys = torch.cat(label_ys, dim=0)
            xts_sample = self.model_ema(xts, ts, label_ys)
            ...
            xts_sample = torch.cat(
                [x[None, ...], xts_sample], dim=0).transpose(0, 1)
            xts_sample = xts_sample.clip(-1, 1)
            xts_sample_grid = make_grid(
                (xts_sample.reshape(-1, *xts_sample.shape[2:]) + 1) / 2, 11)

            self.trainer.loggers[0].log_image(
                'sample_t', [xts_sample_grid], self.trainer.global_step)
            self.trainer.loggers[0].log_image(
                'sample_noise', [xts_grid], self.trainer.global_step)
    ...
    def sample(self):
        with autocast(device_type='cuda', enabled=False):
            ...
            y = torch.randint(0, 1000, (x.shape[0],), device=x.device).long()
            noise = torch.randn_like(x) * self.config['sigma_max']
            x_sample = self.model_ema(noise, x.new_ones([x.shape[0], ], dtype=torch.float32) * self.config['sigma_max'], y)
            x_sample = x_sample.clip(-1, 1)
            x_sample_grid = make_grid((x_sample + 1) / 2)
    ...
    def update_r1_ema_and_aug_p(self, r1_loss):
        ...
        if self._r1_ema > self.config['r1_threshold']:
            self.ada_aug_p += self.config['batch_size'] * \
                self.config['r1_interval'] / self.config['ada_length']
        else:
            self.ada_aug_p -= self.config['batch_size'] * \
                self.config['r1_interval'] / self.config['ada_length']
...
def run(config):
    ...
    trainer = pl.Trainer(
        ...
        ] + ([ModelCheckpoint(path_auxiliary.checkpoint, "checkpoint-{epoch}-{step}", every_n_train_steps=config['save_every'], save_last=False,
    ...
    if rank_zero_only.rank == 0:
        trainer.logger.experiment.config.update(
            get_params_to_record_from_funcion_signature(config, config['function_signature']))
...
def main(
    ...
    r1_interval                    : int       = 16,
    r1_gamma                       : float     = 10,
    r1_threshold                   : float     = 10000000,  # a very large number to disable augmentation
    ...
    max_steps = max_steps * 2  # pytorch_lightning treats an optim invoke as a step, so we need to double the max_steps
    consistency_loss_weight: str = 'none'
    t_rescale = False

    path_auxiliary = PathAuxiliary(
        dataset_path=dataset_path, ckpt_path=ckpt_path, create_path=True, debug=False)
    config = locals()
    config['model_config'] = {
        't_rescale': t_rescale,
